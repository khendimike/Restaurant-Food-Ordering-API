...
class controller {
  static async newFood(req, res, next) {
      ...
      try {
          ...
          if (foundName) {
              ...
              err.status = 406
              err.message = "This food is already added."
              throw err
          }
    ...
    try {
      if(req.user.role === "admin") {
        ...
        if (findUser.role === "admin") {
          return res.status(406).json("User is already an admin")
        }
  ...
  static async makeFoodAvailable(req, res, next) {
    try {
      ...
      if (!findFood) {
        return res.status(404).json(` ${foodToMakeAvaiableOrNot} is not on our menu, add the food first`)
      }
      ...
      if (findFood.available) {
        ...
        return res.status(201).json(`${findFood.name} is not available for order`)
      }
